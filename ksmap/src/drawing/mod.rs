...
fn draw_object_with_offset(
    ...
    let mut rng_flip = ctx.seed.step(RngStep::Flip)
        .salted((ctx.screen_pos, ctx.layer, at_index));
...
fn draw_spritesheet(
    ...
    let mut rng_frame = ctx.seed.step(RngStep::Frame)
        .salted((ctx.screen_pos, ctx.layer, at_index));
    ...
    if let Some(alpha_range) = params.alpha_range.as_ref() {
        let mut rng_alpha = ctx.seed.step(RngStep::Alpha)
            .salted((ctx.screen_pos, ctx.layer, at_index));
...
fn draw_elemental(ctx: &mut DrawContext, curs: Cursor) {
    let mut rng = ctx.seed.step(RngStep::ElementalVariant)
        .salted((ctx.screen_pos, ctx.layer, curs.i));
...
fn draw_with_random_offset(ctx: &mut DrawContext, curs: Cursor, range: RangeInclusive<i64>) {
    let mut rng = ctx.seed.step(RngStep::Offset)
        .salted((ctx.screen_pos, ctx.layer, curs.i));
